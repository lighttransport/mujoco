# MuJoCo USD Physics Annotations Reference
...
MuJoCo has two USD integration paths:

| Path | Library | Physics Schema Support |
|------|---------|----------------------|
| **Native (desktop)** | OpenUSD (Pixar) | Full — all mjcPhysics schemas read/written via `plugin/usd_decoder/` and `src/experimental/usd/` |
| **Web (WASM)** | TinyUSDZ v0.9.9 | Geometry only — mesh extraction + AABB proxy boxes for physics (`web/usd_loader.js`) |
...
## TinyUSDZ (WASM) Support Status

The web demo (`web/usd_loader.js`) uses TinyUSDZ v0.9.9 via npm. Current capabilities:

| Feature | Status | Notes |
|---|---|---|
| Load USD/USDA/USDC/USDZ files | Supported | Via `TinyUSDZLoader.loadAsync()` |
| Extract mesh geometry (vertices, faces) | Supported | `usdScene.getMesh()` |
| Scene-graph traversal with transforms | Supported | `flattenMeshNodes()` recursive walk |
| Y-up to Z-up coordinate conversion | Supported | USD/Three.js Y-up to MuJoCo Z-up |
| AABB bounding-box proxy geoms | Supported | Box geoms computed from mesh bounds |
| Three.js visual mesh creation | Supported | `convertUsdMeshToThreeMesh()` |
| Static/dynamic body classification | Supported | By name list or custom classifier |
| **mjcPhysics schema reading** | **Not yet** | No physics attributes extracted |
| **Joint creation from USD** | **Not yet** | All dynamic bodies get free joints |
| **Actuator setup** | **Not yet** | No actuator prims read |
| **Collision parameters** | **Not yet** | Default condim/solref/solimp used |
| **Material properties** | **Not yet** | No friction/restitution from USD |
| **Tendon/equality constraints** | **Not yet** | Not parsed |
| **Keyframe loading** | **Not yet** | Not parsed |
| **Scene-level options** | **Not yet** | Hardcoded timestep=0.002, gravity=-9.81 |

### Implementation Priority for TinyUSDZ WASM

To support physics annotations in TinyUSDZ WASM, the following work is needed:

1. **Read USD prim metadata/properties** — TinyUSDZ needs API to access custom schema attributes
   (e.g., `mjc:stiffness`, `mjc:condim`) from loaded prims, not just mesh geometry
2. **Map UsdPhysicsJoint prims to MuJoCo joints** — detect revolute/prismatic/spherical joints
   and create corresponding hinge/slide/ball joints via mjSpec API
3. **Apply collision parameters** — read `MjcCollisionAPI` attributes and pass to `MjsGeom`
4. **Apply material properties** — read friction values from `MjcMaterialAPI`
5. **Create actuators** — read `MjcActuator` prims and set up via mjSpec
6. **Scene options** — read `MjcSceneAPI` for timestep, integrator, gravity, etc.
